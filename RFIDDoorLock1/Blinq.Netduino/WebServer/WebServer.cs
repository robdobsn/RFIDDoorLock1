...
namespace Blinq.Netduino.Web
{
    ...
    public class WebServer
    {
        ...
        private bool serverProcessingCommand = false;
        ...
        private void SendResponse(Socket clientSocket, string message, ushort responseCode = 200)
        {
            string responseCodeMessage = "500 Internal Error";
            switch (responseCode)
            {
                case 200:
                    responseCodeMessage = "200 OK";
                    break;
                case 404:
                    responseCodeMessage = "404 not recognised";
                    break;
            }
            string header = "HTTP/1.0 " + responseCodeMessage + "\r\nContent-Type: text; charset=utf-8\r\nContent-Length: " + message.Length.ToString() + "\r\nConnection: close\r\n\r\n";
            try
            {
            ...
            {
                Debug.Print("WebServer::SendResponse::Send-header " + e.Message);
            }
            Thread.Sleep(100);
            try
            {
                clientSocket.Send(Encoding.UTF8.GetBytes(message), message.Length, SocketFlags.None);
            }
            catch (Exception e)
            {
                Debug.Print("WebServer::SendResponse::Send-message " + e.Message + " sending msg " + message);
            }
        }
